...
subject1="Data-Provider ${ftp_user} over `hostname` has files older than 24 hours"
...
cd /home/${ftp_user}/ftproot &> /dev/null
#cd /home/tmishra/app-fte-normalizer &> /dev/null
if [ $? -ne 0 ];then
echo "Home Directory for user ${ftp_user} doen not exits at `date`" >> error.txt
exit
fi
...
echo "File for user ${ftp_user} not Older than 24hours at time `date`" >> ${all_file_path}/message.txt
fi
