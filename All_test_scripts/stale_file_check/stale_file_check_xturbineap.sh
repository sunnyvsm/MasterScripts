...
cd /normalizer-output/xaxisturbine/ap/xtract &> /dev/null
if [ $? -ne 0 ];then
echo "Directory /normalizer-output/xaxisturbine/ap/xtract/ for user xaxisturbine does not exits at `date`" >> error.txt
else
count=`find . -type f  -name ="turbine.*" -mtime  +1  | wc -l`
        ...
        echo "File for user xaxisturbine at location /xaxisturbine/ap/xtract/ not Older than 24hours at time `date`" >> ${all_file_path}/message.txt
        fi
fi
