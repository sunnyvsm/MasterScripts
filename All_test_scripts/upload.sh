...
logDir="/home/lucidmedia/ClickSense/local/logs/"
...
if [ -d ${logDir} ];then
        ...
        do
                if [ `echo "${file}" | egrep -c "^${fileList}*"` -eq 1 ];then
                        bucket="${prefix}lzlogs/userdb/in"
                                else
                        echo "WARN: invalid file type: ${file}"
                        continue
                fi

                _count=`ls | egrep "^${file}\.log\..*" | wc -l`
                count=$(($count + $_count))
                                echo $count

                for log in `ls | egrep "^${file}\.log..*"`
                do
                        if [ `echo ${log} | egrep -c "^${file}\.log\.[0-9]{2}\-[0-9]{2}\-[0-9]{2}\-[0-9]{2}$"` -eq 1 ];then
                        ...
                        do
                                ...
                                if [ "`s3cmd ls s3://${bucket}/${date}/${hour}/${destFile} 2>/dev/null | awk '{print $3}'`" = "`du -sb ${log} 2>/dev/null | awk '{print $1}'`" ];then
                                        echo -en "ok\n"
                                        echo -n "Deleting the logs from local"
                                        #mv ${log} /archive/logs
                                        rm -f ${log}
                                        if [ $? -eq 0 ];then
