...
echo  -e "\n\033[05mHour\033[0m\nPlease Enter the Hour Range(hr1..hr2) , for single hour please enter Single Hour (hr1) "
read hour
...
echo -e "$1\t$2\t$((success *100 /total))\t\t$((twos *100 /total))" >> ocs_success_fail_per.ocs
}


mkdir /tmp/ocs > /dev/null
cd /tmp/ocs
        if [ ${region} == "all" ]; then > /dev/null
...
find -type f -name ocs_success_fail_per.ocs -exec cat {} \; | grep usva | sort -k1 >> /tmp/ocs/total_ocs_count
find -type f -name ocs_success_fail_per.ocs -exec cat {} \; | grep usor | sort -k1 >> /tmp/ocs/total_ocs_count
find -type f -name ocs_success_fail_per.ocs -exec cat {} \; | grep euir | sort -k1 >> /tmp/ocs/total_ocs_count
find -type f -name ocs_success_fail_per.ocs -exec cat {} \; | grep apac | sort -k1 >> /tmp/ocs/total_ocs_count
sed -i '1i Hour\tRegion\tsuccess\t\tfail' /tmp/ocs/total_ocs_count

cat /tmp/ocs/total_ocs_count | mutt -e "set from=${from}" -s "${subject}"  -- "${mail_to}"
if [ $? == 0 ]; then echo "report is mailed to id: ${mail_to}" ; fi
cat /dev/null > /tmp/ocs/total_ocs_count
for  region in ${reg[@]}; do
rm -rf ${region}
done
